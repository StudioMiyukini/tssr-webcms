...
const blocks: PageBlock[] = [
  ...
  note('orange', '🛠️ Dépannage — « Accès refusé » sur l’ISO (VM dupliquée)', '<p>Au démarrage, tu peux tomber sur : <em>« Le compte Service de gestion d’ordinateurs virtuels Hyper-V ne dispose pas des autorisations nécessaires pour ouvrir …<code>.iso</code>. Accès refusé. »</em> C’est une histoire de <strong>permissions NTFS</strong>, pas de configuration de la VM.</p><p>Chaque VM possède une <strong>identité de sécurité propre</strong> (<code>NT VIRTUAL MACHINE\\&lt;GUID&gt;</code>). Quand tu montes l’ISO via l’assistant, Hyper-V ajoute <em>ce GUID</em> à l’ACL du fichier. Mais si tu <strong>dupliques la VM par script/copie</strong> (nouveau GUID) ou si l’ISO est dans un dossier de profil verrouillé (ex. <code>Downloads</code>), la nouvelle VM n’a pas le droit de lire l’ISO → accès refusé.</p><p><strong>Correctif</strong> — autoriser en lecture le groupe qui contient <strong>toutes</strong> les VM (présentes et futures), à faire <strong>une fois par ISO</strong> :</p><div style="overflow-x:auto;margin:6px 0"><code style="display:block;padding:10px 12px;border-radius:8px;background:var(--surface-2);border:1px solid var(--border);font-family:ui-monospace,monospace;font-size:12.5px;white-space:pre">icacls "C:\\chemin\\vers\\image.iso" /grant "*S-1-5-83-0:(RX)"</code></div><p><code>*S-1-5-83-0</code> est le SID du groupe <code>NT VIRTUAL MACHINE\\Virtual Machines</code> (l’ensemble des VM). Place idéalement cette ligne dans ton <strong>script de duplication</strong> pour que chaque nouvelle VM puisse lire l’ISO sans intervention.</p>'),
