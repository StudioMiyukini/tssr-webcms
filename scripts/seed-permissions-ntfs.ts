...
const styleAjout = block('html', { html: `<style>.pn-t{border-collapse:collapse;width:100%;font-size:13px;margin:6px 0}.pn-t th,.pn-t td{border:1px solid var(--border);padding:7px 10px;text-align:left;vertical-align:top}.pn-t th{background:var(--surface-2)}.pn-cmd{font-family:ui-monospace,'Space Mono',monospace;background:var(--surface-2);border:1px solid var(--border);border-radius:8px;padding:10px 12px;margin:8px 0;white-space:pre-wrap;overflow-x:auto;font-size:12.5px;line-height:1.55}</style>` });
...
const blocks: PageBlock[] = [
  ...
  block('heading', { level: 2, text: '🛡️ Le socle : ce qui doit rester quand on coupe l’héritage' }),
  block('html', { html: '<p>Couper l’héritage sur un dossier et ne garder que « le groupe qui doit y accéder » est l’erreur qui immobilise une salle. Ces trois comptes restent, toujours :</p><table class="pn-t"><thead><tr><th>Compte</th><th>Droit</th><th>Sans lui</th></tr></thead><tbody><tr><td><strong>Administrateurs</strong> <code>S-1-5-32-544</code></td><td>Contrôle total</td><td>Plus personne ne peut reprendre la main sur le dossier — il faut passer par une appropriation.</td></tr><tr><td><strong>SYSTEM</strong> <code>S-1-5-18</code></td><td>Contrôle total</td><td>La sauvegarde, l’indexation et l’antivirus perdent l’accès, silencieusement.</td></tr><tr><td><strong>Utilisateurs authentifiés</strong> <code>S-1-5-11</code></td><td>Lecture et exécution, <em>ce dossier seulement</em></td><td>Personne ne peut <em>traverser</em> le dossier parent pour atteindre le sous-dossier auquel il a pourtant droit.</td></tr></tbody></table>' }),
  note('blue', '💡 Pourquoi on les désigne par leur SID', '<p>« Administrateurs » s’appelle « Administrators » sur un Windows anglais. Un script qui les nomme échoue sur la moitié des postes d’une salle ; le <strong>SID</strong>, lui, est le même partout.</p>'),
  note('yellow', '⚠️ Convertir l’héritage, ne pas le supprimer', '<p>La boîte de dialogue propose deux options quand on décoche l’héritage : <strong>Convertir</strong> (copier les droits hérités sur place) ou <strong>Supprimer</strong>. « Supprimer » laisse un dossier <strong>sans aucune autorisation</strong> : plus personne n’entre, y compris pour réparer. En ligne de commande, c’est la différence entre <code>icacls … /inheritance:d</code> et <code>/inheritance:r</code>.</p>'),

  block('heading', { level: 2, text: '🗑️ Les deux suppressions' }),
  block('html', { html: '<p>« Modification » autorise la suppression. Quand on veut permettre d’écrire <em>sans</em> permettre d’effacer, il faut descendre dans les droits avancés — et distinguer deux codes que tout le monde confond :</p><table class="pn-t"><thead><tr><th>Code</th><th>Nom Windows</th><th>Portée</th></tr></thead><tbody><tr><td><code>DE</code></td><td>Suppression</td><td>Supprimer <strong>cet objet</strong> (le fichier, ou le dossier lui-même).</td></tr><tr><td><code>DC</code></td><td>Suppression de sous-dossier et fichier</td><td>Supprimer <strong>le contenu</strong> d’un dossier.</td></tr></tbody></table><p>« Modifier sans suppression » consiste exactement à accorder Modification puis à retirer ces deux-là. Oublier <code>DC</code> laisse l’utilisateur vider le dossier ; oublier <code>DE</code> le laisse supprimer le dossier entier.</p>' }),

  block('heading', { level: 2, text: '👑 Le propriétaire' }),
  block('html', { html: '<p>Chaque fichier a un <strong>propriétaire</strong>, et le propriétaire peut toujours modifier les autorisations — <em>même si toutes les autorisations lui ont été retirées</em>. C’est la porte de secours qui empêche qu’un dossier devienne définitivement inaccessible.</p><p>Un administrateur peut <strong>s’approprier</strong> un dossier (onglet Sécurité → Avancé → Propriétaire → Modifier) et reprendre la main. C’est aussi la raison pour laquelle il ne faut pas créer à l’avance les dossiers destinés à être créés par le système — voir <a href="/pages/profils-itinerants">les profils itinérants</a>, où cette règle décide de tout.</p>' }),

  block('heading', { level: 2, text: '🙈 L’énumération basée sur l’accès (ABE)' }),
  block('html', { html: '<p>Par défaut, un utilisateur <em>voit</em> les dossiers auxquels il n’a pas droit : il ne peut pas les ouvrir, mais leur existence lui est révélée, avec leur nom. L’<strong>ABE</strong>, activée sur le partage, masque ce qu’on ne peut pas ouvrir.</p><p>Ce n’est <strong>pas</strong> une sécurité — les droits NTFS le sont déjà. C’est ce qui évite la question « pourquoi je vois un dossier Direction que je ne peux pas ouvrir ? ».</p>' }),

  block('heading', { level: 2, text: '⌨️ Les commandes' }),
  block('html', { html: `<div class="pn-cmd">REM --- Lire les droits d'un dossier ---\nicacls "E:\\Partages\\Compta"\n\nREM --- Accorder, avec heritage sur les fichiers (OI) et les dossiers (CI) ---\nicacls "E:\\Partages\\Compta" /grant "MIYUKINI\\DL_Compta_M:(OI)(CI)M"\n\nREM --- Modification SANS suppression : on enumere les droits avances ---\nicacls "E:\\Partages\\Compta" /grant "MIYUKINI\\DL_Compta_MND:(OI)(CI)(RD,WD,AD,REA,WEA,RA,WA,X,RC)"\n\nREM --- Couper l'heritage en CONSERVANT les droits herites ---\nicacls "E:\\Partages\\Compta" /inheritance:d\n\nREM --- Retirer un groupe ---\nicacls "E:\\Partages\\Compta" /remove:g "MIYUKINI\\DL_Compta_L\\"</div>` }),
  block('html', { html: `<div class="pn-cmd"># PowerShell : le partage, ses droits, et l'ABE\nNew-SmbShare -Name 'Partages' -Path 'E:\\Partages' -FullAccess 'Utilisateurs authentifies'\nSet-SmbShare -Name 'Partages' -FolderEnumerationMode AccessBased -Force\nGet-SmbShareAccess -Name 'Partages'\n\n# Qui possede ce dossier, et quelles ACE porte-t-il ?\n(Get-Acl 'E:\\Partages\\Compta').Owner\n(Get-Acl 'E:\\Partages\\Compta').Access | Format-Table IdentityReference, FileSystemRights, IsInherited</div>` }),

  note('blue', '🧩 Et les groupes, dans tout ça ?', '<p>Aucun droit ne se pose sur un utilisateur : il se pose sur un <strong>groupe de domaine local</strong>, qui contient un <strong>groupe global</strong>, qui contient les <strong>comptes</strong>. C’est la chaîne <strong>AGDLP</strong>, et c’est ce qui permet d’ajouter un comptable sans toucher à un seul dossier. Voir <a href="/pages/procedure-agdlp">la procédure</a> et le <a href="/pages/constructeur-agdlp">constructeur AGDLP</a>, qui génère les groupes, les dossiers et les scripts de vérification.</p>'),

  note('green', '🎯 À retenir', '<p><strong>Partage</strong> = accès réseau, grossier, sur le point de partage. <strong>NTFS</strong> = toujours, fin, par dossier, avec héritage. En réseau, <strong>le plus restrictif gagne</strong> ; <strong>Refuser &gt; Autoriser</strong>. En pratique : <strong>un seul partage large</strong> + <strong>sécurité NTFS sur des groupes</strong>. Pour aller plus loin : <a href="/pages/lecteurs-reseau">les lecteurs réseau</a>, <a href="/pages/profils-itinerants">les profils itinérants</a>, <a href="/pages/procedure-agdlp">AGDLP</a>.</p>'),
];
