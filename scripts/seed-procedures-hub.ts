...
const CATEGORIES: { id: string; icon: string; label: string }[] = [
  { id: 'depannage', icon: '🩺', label: 'Diagnostic & dépannage' },
...
const PROCEDURES: Proc[] = [
  { slug: 'depannage', icon: '🩺', title: 'Dépannage — méthode & diagnostics', desc: 'La démarche de diagnostic couche par couche (modèle OSI) et les pannes courantes : liaison, IP/passerelle, DNS, pare-feu (ping), services… avec les commandes de test à chaque étape.', tags: ['Dépannage', 'Méthode', 'OSI', 'Réseau'], cat: 'depannage' },
  ...
  { slug: 'tp-manipulation-fichiers', icon: '📂', title: 'TP — Recherche : manipulation de fichiers', desc: 'Documenter dix-sept commandes de fichiers et six de dossiers : à quoi elles servent, deux paramètres, un exemple. Repères en fin de page, dont les trois pièges qui ne se voient qu’une fois : cp qui écrase sans demander, mv qui sert à deux choses, rmdir qui refuse un dossier non vide.', tags: ['Linux', 'Commandes', 'TP'], cat: 'linux' },
  { slug: 'tp-utilisateurs-recherche', icon: '🔎', title: 'TP — Recherche : utilisateurs et droits', desc: 'Quinze commandes et trois fichiers de gestion des comptes, à documenter. Avec la réponse à la question du TP — pourquoi useradd et adduser existent tous les deux — et le piège de /etc/group, où le groupe principal n’apparaît jamais.', tags: ['Linux', 'Utilisateurs', 'Droits', 'TP'], cat: 'linux' },
  { slug: 'tp-utilisateurs', icon: '👥', title: 'TP — Utilisateurs', desc: 'Créer, modifier et supprimer comptes et groupes en vérifiant dans /etc/passwd et /etc/group. Les trois surprises : le compte qu’on ne peut pas ouvrir, le prompt réduit à un dollar, et la découverte finale — tout appartient à root.', tags: ['Linux', 'Utilisateurs', 'TP'], cat: 'linux' },
  { slug: 'tp-utilisateurs-droits', icon: '🔐', title: 'TP — Utilisateurs et droits', desc: 'Remettre d’aplomb l’arborescence du TP précédent avec chown, chgrp et chmod. Méthode de traduction phrase → rwx → octal, et le piège du -R qui rend les fichiers exécutables.', tags: ['Linux', 'Droits', 'chmod', 'TP'], cat: 'linux' },
  { slug: 'tp-ssh-securisation', icon: '🔑', title: 'TP — Sécuriser SSH : port et clés', desc: 'Déplacer le port d’écoute, puis remplacer le mot de passe par une paire de clés générée avec PuTTYgen. Avec les deux échecs silencieux du TP — les permissions de ~/.ssh et le format de la clé publique — et la règle qui évite de s’enfermer dehors.', tags: ['Linux', 'SSH', 'Sécurité', 'TP'], cat: 'linux' },
  { slug: 'tp-archivage', icon: '🗜️', title: 'TP — Archivage et compression', desc: 'Vingt manipulations de tar, gzip et zip, puis un aller-retour Windows ↔ Linux par le panneau SFTP de MobaXterm. Avec les quatre commandes qui échouent si on suit l’énoncé littéralement — dont lire un fichier dans une archive compressée.', tags: ['Linux', 'tar', 'gzip', 'TP'], cat: 'linux' },
  { slug: 'tp-config-reseau-statique', icon: '🖧', title: 'TP — Configuration réseau : IP statique', desc: 'Relever ce que le DHCP a donné (ip a, ip r), le figer dans /etc/network/interfaces, appliquer sans se couper. Avec les deux messages d’erreur du TP — celui qui est inoffensif et celui qui bloque — et le configurateur pour vérifier avant d’écrire.', tags: ['Linux', 'Debian', 'Réseau', 'TP'], cat: 'linux' },
  ...
  { slug: 'procedure-vlan-packet-tracer', icon: '🧩', title: 'Mettre en place des VLAN', desc: 'De la topologie vierge à deux VLAN qui communiquent : plan d’adressage, création des VLAN, ports access, trunk, puis les deux méthodes de routage inter-VLAN détaillées pas à pas — router-on-a-stick et switch multicouche (SVI). Commandes Cisco, contrôle après chaque étape, pannes classiques.', tags: ['Cisco', 'Packet Tracer', 'VLAN', 'Réseau'], cat: 'cisco' },
