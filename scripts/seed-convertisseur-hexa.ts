...
const blocks: PageBlock[] = [
  ...
  block('html', { html: '<p>Colle un <strong>dump hexadécimal</strong> (par exemple les octets d’une trame capturée dans Wireshark) : l’outil le convertit en <strong>texte UTF-8/ASCII</strong>, <strong>décimal</strong> et <strong>binaire</strong>. Pour du <strong>binaire</strong>, il extrait aussi les <strong>chaînes lisibles</strong> (comme la commande <code>strings</code>) et affiche une <strong>vue hexdump</strong> (offset · hexa · ASCII) — pratique pour repérer un nom, un identifiant ou la « signature » d’un fichier. Il fait aussi l’inverse (<strong>Texte → hexa</strong>) et décode le <strong>Base64</strong>. Le collage tolère les espaces, retours à la ligne, <code>0x</code> et <code>:</code>.</p>' }),
